...
function renderProducts(){
  ...
  $("products").innerHTML=list.map(p=>{
    ...
    return `<article class="card">
      <img src="${safeUrl(p.image_url)}" alt="${escapeHtml(p.name)}" onerror="this.src='https://placehold.co/600x600?text=Product'">
      ...
      <br><br><button onclick="addToCart(${p.id})">Add to Cart</button></div></article>`;
  }).join("");
}
$("search").addEventListener("input",renderProducts);

function addToCart(id){const p=products.find(x=>x.id===id);if(!p)return;const row=cart.find(x=>x.id===id);if(row)row.qty++;else cart.push({...p,qty:1});renderCart();openCart();}
function renderCart(){
  $("cartCount").textContent=cart.reduce((s,x)=>s+x.qty,0);
  $("cartItems").innerHTML=cart.length?cart.map(x=>`<div class="cart-row">
    <img src="${safeUrl(x.image_url)}" onerror="this.src='https://placehold.co/100x100?text=Product'">
    <div style="flex:1"><b>${escapeHtml(x.name)}</b><div>₹${money(x.discount_price??x.price)} × ${x.qty}</div>
    <button onclick="removeItem(${x.id})">Remove</button></div></div>`).join(""):"<p>Your cart is empty.</p>";
  $("cartTotal").textContent=money(cart.reduce((s,x)=>s+(Number(x.discount_price??x.price)+Number(x.delivery_charge||0))*x.qty,0));
}
...
$("checkoutBtn").onclick=()=>alert("Checkout system next step mein add karenge: address + COD + online payment.");
